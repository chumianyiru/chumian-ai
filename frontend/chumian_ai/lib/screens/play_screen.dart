...
class _PlayScreenState extends State<PlayScreen> {
  ...
  Future<void> _load() async {
    try {
      final results = await Future.wait([
        ExploreService().fetchTemplates('play'),
  ...
  Widget build(BuildContext context) {
    return GradientScaffold(
      appBar: AppBar(title: const Text('玩乐')),
      body: _loading
                    ...
                    sliver: SliverList(
                      delegate: SliverChildBuilderDelegate(
                        (context, index) {
                          ...
                          return _PlayAgentCard(agent: agent);
                        },
...
class _PlayTemplateCard extends StatelessWidget {
  final Template template;
  final VoidCallback onTap;

  const _PlayTemplateCard({required this.template, required this.onTap});
...
class _PlayAgentCard extends StatelessWidget {
  final Agent agent;

  const _PlayAgentCard({required this.agent});
